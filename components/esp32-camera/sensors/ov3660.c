...
static int wait_sccb_ready(sensor_t *sensor, int timeout_ms)
{
    ...
    while (((xTaskGetTickCount() - start) * portTICK_PERIOD_MS) < (TickType_t)timeout_ms) {
        int v = read_reg(sensor->slv_addr, 0x300A);
        if (v == 0x36) {
            return 0;
        }
        vTaskDelay(pdMS_TO_TICKS(5));
    }
    ESP_LOGE(TAG, "SCCB ready probe timed out after %d ms", timeout_ms);
    return -1;
}
...
static int set_pll(sensor_t *sensor, bool bypass, uint8_t multiplier, uint8_t sys_div, uint8_t pre_div, bool root_2x, uint8_t seld5, bool pclk_manual, uint8_t pclk_div){
    ...
    if (ret == 0) {
        ...
        //   REFIN in [7,11] MHz  -> code 010 (0x20)
        //   REFIN > 11 MHz       -> code 011 (0x30)
        const int pre_div_val_x10[] = { 10, 15, 20, 30 }; // pre_div idx -> multiplier *10
        const int refin_x10 = (sensor->xclk_freq_hz / 100000) * 10 / pre_div_val_x10[pre_div & 0x3];
        uint8_t cp_code;
        if (refin_x10 < 70) {
            cp_code = 0x10;
        } else if (refin_x10 <= 110) {
            cp_code = 0x20;
        } else {
            cp_code = 0x30;
        }
        ret = write_reg(sensor->slv_addr, SC_PLLS_CTRL2, cp_code | (sys_div & 0x0f));
    }
    if (ret == 0) {
    ...
    if (wait_sccb_ready(sensor, 50) != 0) {
        ESP_LOGE(TAG, "SCCB not ready after PLL config (sensor may not have locked)");
        return -1;
    }
    return 0;
}
...
static int set_framesize(sensor_t *sensor, framesize_t framesize)
{
    ...
    if (sensor->pixformat == PIXFORMAT_JPEG) {
        ...
        if (framesize == FRAMESIZE_QXGA) {
            // QXGA (2048x1536) needs higher internal bandwidth; keep the upstream default.
            mult = 24; pre_div_idx = 3; pclk_div = 8;
        } else if (xclk_mhz >= 16 && xclk_mhz <= 27) {
            // Recommended XCLK range: pre_div=/2, REFIN = xclk/2 in [8,13.5] MHz.
            // mult chosen so that SYSCLK = xclk*mult/8 stays as close to 50 MHz as possible.
            static const uint8_t mult_tab[] = {
                /* 16 */ 25, /* 17 */ 24, /* 18 */ 22, /* 19 */ 21, /* 20 */ 20,
                /* 21 */ 19, /* 22 */ 18, /* 23 */ 17, /* 24 */ 17, /* 25 */ 16,
                /* 26 */ 15, /* 27 */ 15,
            };
            mult        = mult_tab[xclk_mhz - 16];
            pre_div_idx = 2;
            pclk_div    = 10;
        } else if (xclk_mhz >= 28 && xclk_mhz <= 30) {
            // XCLK outside the datasheet recommended range 6-27 MHz: use pre_div=/3
            // so that REFIN stays within [9.3,10] MHz.
            static const uint8_t mult_tab[] = {
                /* 28 */ 21, /* 29 */ 21, /* 30 */ 20,
            };
            mult        = mult_tab[xclk_mhz - 28];
            pre_div_idx = 3;
            pclk_div    = 10;
        } else {
            ESP_LOGW(TAG,
                "XCLK %d MHz outside tuned PLL range [16,30]; SYSCLK may drift from 50 MHz target",
                xclk_mhz);
            mult = 30; pre_div_idx = 3; pclk_div = 10;
        }

        ret = set_pll(sensor, false, mult, 1, pre_div_idx, false, 0, true, pclk_div);
    } else {
