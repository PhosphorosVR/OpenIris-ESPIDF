...
void FanManager::setup()
{
    ...
    const uint32_t dutyCycle = (cfg.fan_pwm_duty_cycle * 255) / 100;

    ESP_LOGI(FAN_MANAGER_TAG, "Setting up fan PWM on GPIO %d, freq=%d Hz, duty=%lu", static_cast<int>(fan_pin), freq, dutyCycle);
...
void FanManager::setFanDutyCycle(uint8_t dutyPercent)
{
    ...
    ESP_LOGI(FAN_MANAGER_TAG, "Updating fan duty to %u%% (raw %lu)", dutyPercent, dutyCycle);
